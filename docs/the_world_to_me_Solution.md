...
|---|---|---|---|
| 完全睁眼 | ≥ 0.80 | 实时 | 像素采样为 ~14400 颗大圆（**v3.8 size 40px**，cell 3.3 倍 → 印象派厚涂）；最大抖动半径 60px；**径向 alpha 衰减**（v3.7：中心 1.0 / 四角 ~0.4）；sampleBuf POSTERIZE 4 级；颜色轻去饱和 0.25 |
...
### Decision #9：粒子也走 POSTERIZE，桥接两态美学语言（v3.6）

v3.5 闭眼态推到了毕加索立体主义平涂色块，但睁眼粒子仍是 256 级真实采样的柔粒子。用户反馈"睁眼闭眼割裂"——色彩颗粒度（256 级 vs 27 级）差太远，是两套美学语言。

**问题诊断**

| 维度 | 睁眼粒子（v3.5 之前） | 闭眼 echo（v3.5） | 是否割裂 |
|---|---|---|---|
| 色彩颗粒度 | 256 级真实采样 | POSTERIZE 27 色平涂 | **割裂** |
| 形态 | 圆点（动） | 平涂色块（定） | 应有反差（这是核心隐喻） |
| 色彩来源 | 摄像头采样 | 摄像头采样 + 毕加索色板染色 | 应有反差（"看见 vs 想象"）|
| 动态 | 50px 抖动 | 共享 trail 缓慢旋转 | 应有反差（"流动 vs 凝固"）|

**核心反差**（睁=动/看见 vs 闭=定/想象）必须保留，不能动。但**色彩颗粒度**这层应该统一——这是把整部作品装进同一个美学语言的钥匙。

**实现（2 处改动，加起来 3 行代码）**

```js
// CFG（新增 1 个、改 1 个）
DESATURATION_OPEN: 0.25,             // 0.45 → 0.25：让 POSTERIZE 后的色块色彩留下来
PARTICLE_POSTERIZE_LEVEL: 4,         // 4 级 = 64 色，比 echo 的 3 级稍温和

// refreshSampleBuf 末尾追加
sampleBuf.filter(POSTERIZE, CFG.PARTICLE_POSTERIZE_LEVEL);
```

**为什么级数不同（粒子 4 / echo 3）**

- echo 是"过去定格的几何切片"——3 级最块、最毕加索、最适合"想象"
- 粒子是"流动的当下"——4 级仍平涂但保留更多色阶，仍能看清世界
- 形成"睁=细一点的平涂 / 闭=粗一点的平涂"——同语言但有层次

**为什么粒子仍用摄像头采样色而不是用毕加索色板替换**

桥接的是**色彩颗粒度（POSTERIZE 共享）**而不是色板。色板是闭眼态的"想象的颜色"，睁眼态如果也用毕加索色板替换，"看见 vs 想象"的语义就破坏了——画面会变成"两态都是想象，毕加索效果秀场"。克制是关键。

**为什么 DESATURATION_OPEN 要从 0.45 调到 0.25**

POSTERIZE 后画面已经从 256 色压到 64 色——色阶损失自带"减少视觉信息量"的效果，不再需要重度去饱和来加强"看不清"。继续 0.45 去饱和会让平涂色块变成灰色块，丢失立体主义的色彩力度。0.25 留一些灰化，保留"睁眼=偏冷"的语义但不抹杀色彩。

**性能 / 内存**

- `sampleBuf` 仅 `160×90 = 14400 px`，每帧 `filter(POSTERIZE, 4)` < 0.1ms，可忽略
- 内存零变化

**风险**

- POSTERIZE 把人脸的明度阶减少，可能让面部细节（鼻翼/眼睑边）变得突兀 —— 实测如发现，调 `PARTICLE_POSTERIZE_LEVEL: 5`（每通道 5 级 = 125 色，温和回升）
- 0.25 去饱和 + 平涂色块可能让画面"太鲜艳"违背"看不清的现实"语义 —— 实测如发现，调回 0.35

### Decision #10：粒子径向焦点衰减——体现"前路迷茫"（v3.7）

v3.6 桥接了两态色彩颗粒度，但睁眼粒子仍是全画面均匀分布、均匀 alpha——**没有距离感 / 焦点感**，再小也只是均匀的细沙，不是"迷茫"。用户反馈："粒子要小一点，体现前路迷茫的视觉效果"。

**视觉语言诊断**

| "前路迷茫"含义 | 粒子语言对应 |
|---|---|
| 颗粒像沙尘/星尘 | 粒子尺寸调小 + 抖动半径略增 |
| **近清远糊（焦点感）** | **径向 alpha 衰减**：中心粒子原透明度，越往外越淡 |
| 焦点收向中央 | 同上的副作用——"管隧视野" |

物理隐喻：浅景深 / 大雾天能见度梯度 / 蒸汽机车风挡前的雾——眼前还有一点焦点，远处溶进虚无。

**实现（3 处改动）**

```js
// CFG（改 3 / 加 1）
PARTICLE_SIZE_BASE: 1.5,             // 4 → 1.5
PARTICLE_SIZE_OPEN: 3,               // 7 → 3，细颗粒感
MAX_JITTER_PX:      60,              // 50 → 60，颗粒更迷散
RADIAL_FOCUS_FALLOFF: 0.6,           // alpha *= 1 - 0.6 * (dist/maxDist)^2

// drawParticleLayer 内每颗粒子加：
const dx = cellCx - cx, dy = cellCy - cy;
const distNorm2 = (dx*dx + dy*dy) * invMaxDist2;
const focus = 1 - mix * 0.6 * distNorm2;  // mix 联动：闭眼态不衰减
fill(r, g, b, pa * focus);
```

**alpha 衰减曲线（POW=2 平方衰减）**

| 位置 | distNorm² | focus | 实际 alpha |
|---|---|---|---|
| 画面中心 | 0.00 | 1.00 | 200 |
| 半径中点 | 0.25 | 0.85 | 170 |
| 1/2 边缘 | 0.50 | 0.70 | 140 |
| 半径处（边缘中央） | ~0.75 | 0.55 | 110 |
| 四角 | 1.00 | 0.40 | 80 |

非线性的平方曲线让"中心还算清，离开就快速糊掉"——比线性更像真正的景深和雾。

**为什么 falloff 跟 mix 联动**

睁眼度 `mix=0`（闭眼）时 falloff=0，粒子层不衰减——但闭眼时粒子层 alpha 本身已 → 0 看不见，所以是冗余但安全的设计：避免半睁半闭过渡区间出现"既糊又暗"叠加过头。

**为什么用 POW=2 平方衰减而不是更陡的曲线**

- POW=2 用 `dx*dx + dy*dy` 直接得到 distNorm²，跳过 sqrt/pow → 14400 颗粒子 ≈ 0.05ms / 帧
- POW=4（更陡的"管隧"）每颗粒子要再一次乘法，开销翻倍但收益不大
- POW=1（线性）边缘衰减到 0.4，但中心区域 0.85 已经开始变糊——焦点不够锐
- POW=2 是 sweet spot：中心 1.0 / 半径中 0.85 / 四角 0.4，焦点和迷雾都成立

**搜索结论**

- 项目历史：`git log -S 'distNorm'` / `RADIAL_FOCUS` 无既有调用，首次使用
- 项目内：vignette 烘焙逻辑（`drawVignette` 函数）也是径向衰减但作用在最终合成层，跟粒子层逻辑独立
- 外部：径向衰减是基础数学，无需 web 搜

**与 vignette 协同**

闭眼态本来就有 vignette（`drawVignette`），是合成层最上方的整体压暗。睁眼态新增的粒子径向焦点是**逐粒子 alpha** 而不是合成层叠加——本质不同：
- vignette = 黑色径向叠加（让暗的更暗）
- 粒子径向焦点 = 透明度衰减（让边缘粒子更稀）

睁眼态画面没有 vignette 层（mix > 0.98 时跳过 drawMemoryLayer），不会冲突。

**性能 / 内存**

- 每颗粒子新增 4 次浮点乘法 + 1 次减法 + 1 次预存常量取用 → 14400 颗 ≈ 0.05ms / 帧
- 内存零变化
- 粒子数不变（`SAMPLE_W/H` 未动）

**风险**

- 四角 alpha=80 可能让画面"四个黑角"过强 —— 实测如发现，调 `RADIAL_FOCUS_FALLOFF: 0.4`（四角抬到 ~120）
- 粒子 size=3 可能在 4K 屏上显得太细 —— 实测如发现，调到 4-5

> **实测后路线变更**：v3.7 实测下来"细沙迷雾"路线和闭眼态毕加索平涂色块的"块"感反差太大——闭眼是大色块、睁眼是细沙尘，仍有"颗粒度割裂"。v3.8 反向取大粒子重叠（印象派厚涂），让两态都有"块/涂"感，详见 Decision #11。径向焦点衰减保留并复用——大粒子下边缘衰减效果更强，色块溶解感更显。

### Decision #11：粒子反向放大，统一为印象派厚涂（v3.8）

v3.7 把粒子调到 3px 想做"细沙迷雾"——技术上做到了，但视觉上和闭眼态毕加索 27 色平涂大色块仍然冲突：闭眼"块"，睁眼"沙"，颗粒度依然割裂。用户实测后反向探索："粒子调的非常大给我看看"——大粒子高度重叠的视觉反而成立。

**问题诊断（v3.7 路线的问题）**

| 维度 | 闭眼 echo | 睁眼粒子 v3.7 | 是否同语言 |
|---|---|---|---|
| 形态 | 27 色平涂大色块 | 64 色 3px 细沙点 | **形态割裂**（"块" vs "点"）|
| 色彩颗粒度 | POSTERIZE 3 级 | POSTERIZE 4 级 | 已统一（v3.6 桥接）|
| 整体视觉印象 | 立体主义、油画感 | 沙尘、像素马赛克 | **画种感割裂** |

v3.6 已经统一了"色彩颗粒度"，但**形态颗粒度**仍然完全两套语言。

**反向破局：印象派厚涂**

把 `PARTICLE_SIZE_OPEN` 从 3 拉到 **40**（cell 才 12px，粒子直径是 cell 的 3.3 倍）→ 14400 颗大圆高度重叠。配合：

- POSTERIZE 4 级：每颗大圆色彩离散
- 径向 alpha 衰减（v3.7 已加，复用）：中央色彩浓厚密集，边缘色块稀薄漂浮
- 60px 抖动：大圆在乱晃 → 色块在涂抹
...
| 大小 | 视觉感受 | 艺术参考 |
|---|---|---|
| size = 3 | 沙尘 / 像素马赛克 | (v3.7) |
| size ≈ cellW = 12 | 紧凑色块拼贴 | 修拉点彩派 |
| size = 20-30 | 色块互相重叠 | 莫奈、毕沙罗印象派 |
| **size = 40（当前 v3.8）** | **大色块高度重叠抹涂** | **晚期莫奈睡莲、马克·罗斯科色域绘画** |

**与闭眼态的关系**

| 维度 | 闭眼 echo | 睁眼粒子 v3.8 |
|---|---|---|
| 形态 | 平涂色块（不动） | 大色块抹涂（在动） |
| 色彩颗粒度 | POSTERIZE 27 色 | POSTERIZE 64 色 |
| 整体印象 | 立体主义平面化 | 印象派厚涂 |
| **共同语言** | **油画感、块状感、色彩涂抹** | ✓ 统一 |
| **核心反差** | **凝固定格 / 流动抹涂** | ✓ 保留 |

整部作品现在都在"绘画"美学语言里——一边是立体主义（记忆=被几何分割的色面），一边是印象派（现实=被光线涂抹的色块）。"看见 vs 想象"的反差仍在，但同处一个画框语言里。

**实现（1 处改动）**

```js
PARTICLE_SIZE_BASE: 8,          // 1.5 → 8
PARTICLE_SIZE_OPEN:  40,        // 3 → 40
// 其他参数 v3.7 保持：MAX_JITTER_PX=60 / RADIAL_FOCUS_FALLOFF=0.6 / POSTERIZE=4
```

**为什么 size 跨度这么大（8↔40）**

闭眼端 BASE=8 不是真的能看见——闭眼时粒子层 mix→0，alpha 已 → 0。BASE 主要影响**半睁半闭过渡区间**：从大涂抹（睁）→ 小涂抹（半闭）→ 消失（闭）。BASE=8 让过渡区间逐渐缩小色块，不至于"刚开始闭眼粒子还很大"突兀切换。

**性能验证**

- 14400 颗 ellipse(40, 40)：每颗的画素面积是 size=3 时的 178 倍
- 但 fill+ellipse API 调用次数不变，GPU 光栅化"大圆"和"小圆"成本接近
- 60fps 实测无掉帧（用户实测）
- 内存零变化

**风险**

- 4K 屏上 cell 是 24px，size=40 仅 1.66 倍重叠 → "厚涂感"会减弱。如发现，按屏幕尺寸动态算 `OPEN = max(30, cellW * 3.3)`
- 粒子层 trail alpha 22 在大粒子下叠加可能让"前一帧残影"过厚 —— 实测中如发现糊成纯色，调 `TRAIL_ALPHA_OPEN: 35` 加大每帧黑底覆盖
- 大粒子在低端 GPU 可能有抗锯齿瓶颈 —— 暂未实测，建议 14400×40px ellipse 在集成显卡上观察 fps
...
const CFG = {
  ...
  PARTICLE_SIZE_BASE: 8,                   // v3.8 印象派厚涂（Decision #11）
  PARTICLE_SIZE_OPEN: 40,                  // v3.8 cell 3.3 倍 → 大圆高度重叠
  MAX_JITTER_PX:    60,                    // v3.7 50→60
  TRAIL_ALPHA_OPEN: 22,
  TRAIL_ALPHA_CLOSE: 80,
  DESATURATION_OPEN: 0.25,                 // v3.6 调小，让 POSTERIZE 色块的色彩留下来
  PARTICLE_ALPHA: 200,
  PARTICLE_POSTERIZE_LEVEL: 4,             // v3.6 桥接两态色彩颗粒度（Decision #9）
  RADIAL_FOCUS_FALLOFF: 0.6,               // v3.7 径向焦点（Decision #10）
...
- Decision #9：粒子也走 POSTERIZE，桥接两态美学语言 v3.6（粒子 sampleBuf POSTERIZE 4 级 + 去饱和从 0.45 降到 0.25；睁=细平涂/闭=粗平涂同语言，仍保留"看见 vs 想象"的核心反差）
- Decision #10：粒子径向焦点衰减体现"前路迷茫" v3.7（粒子 size 7→3 + jitter 50→60 + 径向 alpha 平方衰减 falloff=0.6；中心 1.0、四角 0.4 → 浅景深 / 大雾天能见度梯度的视觉隐喻）
- Decision #11：粒子反向放大成印象派厚涂 v3.8（size 3→40，cell 3.3 倍高度重叠；推翻 v3.7 细沙路线，把睁眼粒子从"沙尘像素马赛克"拉到"莫奈睡莲式色域涂抹"，与闭眼立体主义平涂同处"绘画"语言；径向衰减保留并复用）
